...
int main(int argc, char *argv[] ) 
 { 
    ...
    int locH = H/nbp;
    auto iters = computeMandelbrotSet( W, H, rank, nbp, locH, maxIter);
    std::vector<int> globPixels(W*H);
    MPI_Gather(iters.data(),W*locH,MPI_INT,
                globPixels.data(),W*locH,MPI_INT,
                0,globComm);
    if (rank==0){
        savePicture("mandelbort.tga",W,H,globPixels,maxIter);
        end = MPI_Wtime();
