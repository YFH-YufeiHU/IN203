...
int main( int nargs, char* argv[] )
{
    ...
    int  weightRows=N/nbp;
    std::vector<double> locResult(weightRows);
    Matrix locA(weightRows,N);
    std::vector<int> recvbuff(weightRows*N);
    std::vector<int> sendbuff(N*N);
    if(rank==0){
        for(int i=0;i<N;++i){
            for(int j=0;j<N;++j){
                sendbuff.at(i*N+j)=A(i,j);
            }
        }
    }
    
    MPI_Scatter(sendbuff.data(),weightRows*N,MPI_INT,
        recvbuff.data(),weightRows*N,MPI_INT,
        0,//root
        globComm);

    for(int i=0;i<weightRows;++i){
        for(int j=0;j<N;++j){
            locA(i,j) = recvbuff.at(i*N+j);
        }
    }
    locResult = locA*u;
    std::cout<<locResult<<std::endl;

    MPI_Gather(locResult.data(),locResult.size(),MPI_DOUBLE,
                v.data(),locResult.size(),MPI_DOUBLE,
                0,//root
                globComm);
    ...
    if(rank==0){
        ...
        std::vector<double> tValue(N);
        tValue=A*u;
        std::cout<<"true value = "<<tValue<<std::endl;
        if(tValue==v){
